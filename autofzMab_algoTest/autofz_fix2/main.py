...
def thread_update_fuzzer_log(fuzzers):
    logger.info('thread_update_fuzzer_log')
...
class Schedule_Base(SchedulingAlgorithm):
    ...
    def focus_cpu_assign(self, new_cpu_assign, focus_time: int) -> bool:
        ...
        logger.info(f'focus_total: { focus_total}, focus_time: {focus_time}')
        ...
        logger.info(f'round {self.round_num} focus priority - run_fuzzers : {run_fuzzers}')
        ...
        for fuzzer in run_fuzzers:
            ...
            logger.info(f'round {self.round_num} focus after -  focus_fuzzer : {fuzzer}, bitmap : {focus_fuzzer_info_bitmap}, run_time : {t}')
...
class Schedule_Autofz(Schedule_Base):
    ...
    def one_round(self):
        ...
        logger.info(f'confirm early exit condition : {has_winner}')
        ...
        for fuzzer in self.fuzzers:
            logger.info(f'pick before fuzzer: {fuzzer}, picked_time : {self.picked_times[fuzzer]}')

        for fuzzer in picked_fuzzers:
           self.picked_times[fuzzer] += 1

        for fuzzer in self.fuzzers:
            logger.info(f'pick after fuzzer: {fuzzer}, picked_time : {self.picked_times[fuzzer]}')
        ...
        logger.info(f'dynamic_prep_time_round: {self.dynamic_prep_time_round}, dynamic_focus_time_round: {self.dynamic_focus_time_round}')
        ...
        logger.info(f'focus_elasp : { focus_elasp }')
        ...
        for fuzzer in FUZZERS:
            ...
            logger.info(f'round {self.round_num} end result - fuzzer : { fuzzer }, total run time: {self.tsFuzzers[fuzzer].total_runTime}, prep run time : {self.tsFuzzers[fuzzer].prep_runTime}, focus run time: {self.tsFuzzers[fuzzer].focus_runTime},round total run time: {total_roundRunTime}, after_prep_global_bitmap: {after_prep_global_bitmap}')
        ...
        logger.info(f'round {self.round_num} end result - previous_bitmap: {previous_bitmap}, current_bitmap: {current_bitmap}')
...
def main():
    ...
    for fuzzer in FUZZERS:
        ...
        logger.info(f'fuzzer : { fuzzer }. total_runTime: { tsFuzzers[fuzzer].total_runTime }, prep_runTime: {tsFuzzers[fuzzer].prep_runTime}, focus_runTime: {tsFuzzers[fuzzer].focus_runTime}')
