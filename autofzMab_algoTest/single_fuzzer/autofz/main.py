...
class Schedule_Focus(Schedule_Base):
    ...
    def main(self):
        while True:
            ...
            global_bitmap = fuzzerInfo['global_bitmap'].count()
            current_unique_bug = fuzzerInfo['global_unique_bugs']['unique_bugs']
            current_bitmap = fuzzerInfo['bitmap'][self.focus].count()
            logger.info(f'main 200 - total_run_time : {self.run_time}')
            logger.info(f'main 201 - global bitmap : {global_bitmap}')
            logger.info(f'main 202 - current unique bug : {current_unique_bug}')
            logger.info(f'main 203 - current bitmap : {current_bitmap}')
            self.one_round()
            self.post_round()
