...
class fuzzer():
    def __init__(self):
        ...
        self.prep_runTime = 0
        self.focus_runTime =0

def selectFuzzer(fuzzers):
    logger.info('thomps 001 - Select fuzzer')
    selectedFuzzers =[]
    for value in fuzzers.values():
        value.prob = np.random.beta(value.S, value.F, size = 1)
        logger.info(f'thomps 02 - Success: { value.S }, Fail : {value.F}, Prob: { value.prob }')
    max_prob_fuzzer = max(fuzzers, key=lambda key: fuzzers[key].prob)
    logger.info(f'thomps 003 - max_prob_fuzzer: { max_prob_fuzzer }')
    selectedFuzzers.append(max_prob_fuzzer)
    logger.info(f'thomps 004 - selected Fuzzers: {selectedFuzzers}')
...
def updateFuzzerCount(tsfuzzer, selected_fuzzers, criteria):
    for selected_fuzzer in selected_fuzzers:
        ...
        if criteria == 1:
            fuzzer.S = fuzzer.S + 1
            logger.info('thomps 005 - Update fuzzer function - success')
        else:
            fuzzer.F = fuzzer.F +1
            logger.info('thomps 006 -Update fuzzer function - fail')
